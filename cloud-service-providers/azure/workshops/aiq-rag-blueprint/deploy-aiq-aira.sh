...
AZ_MONITOR_WORKSPACE_NAME=${AZ_MONITOR_WORKSPACE_NAME:-aiq-rag-workshop}
GRAFANA_NAME=${GRAFANA_NAME:-aiq-rag-workshop-${RANDOM}}
################################################################################
...
Possible verbs are:
    ...
    setup-infra    Create AKS cluster, Azure Monitor, Grafana, and GPU node pool.
...
Environment variables (with defaults):
    ...
    Azure Monitor and Grafana:
    AZ_MONITOR_WORKSPACE_NAME=${AZ_MONITOR_WORKSPACE_NAME}
...
print_header() {
  ...
  echo "  Monitor Workspace:  $AZ_MONITOR_WORKSPACE_NAME"
...
create_monitor_workspace() {
  log "Creating Azure Monitor Workspace $AZ_MONITOR_WORKSPACE_NAME..."

  if az monitor account show --resource-group "$RESOURCE_GROUP" --name "$AZ_MONITOR_WORKSPACE_NAME" >/dev/null 2>&1; then
    ...
    log "Azure Monitor Workspace created successfully"
  fi
  ...
  log "Azure Monitor Workspace ID: $AZ_MONITOR_WORKSPACE_ID"
}

create_grafana_instance() {
  log "Creating Azure Managed Grafana instance $GRAFANA_NAME..."

  # Check if extension is installed
  if ! az extension show --name amg >/dev/null 2>&1; then
    log "Installing Azure Managed Grafana (amg) extension..."
    az extension add --name amg
  fi

  if az grafana show --name "$GRAFANA_NAME" --resource-group "$RESOURCE_GROUP" >/dev/null 2>&1; then
    log "Grafana instance already exists"
  else
    ...
    az grafana create \
      ...
      --resource-group "$RESOURCE_GROUP" \
      --location "$REGION"
    log "Grafana instance created successfully"
  fi
  ...
  log "Grafana Resource ID: $GRAFANA_RESOURCE_ID"
}
...
import_grafana_dashboard() {
  ...
  # Create folder
  if az grafana folder create \
    --name "$GRAFANA_NAME" \
    --title AIQ-RAG-Workshop \
    --resource-group "$RESOURCE_GROUP" >/dev/null 2>&1; then
    log "Grafana folder created"
  else
    log "Grafana folder already exists or could not be created"
  fi

  # Import dashboard
  if [ -f "manifests/consolidated-nim-dashboard-example.json" ]; then
    ...
    log "WARNING: Dashboard file not found at manifests/consolidated-nim-dashboard-example.json"
  fi
}
...
get_aks_credentials() {
  ...
  az aks get-credentials --resource-group "$RESOURCE_GROUP" --name "$CLUSTER_NAME" --overwrite-existing --file cluster.config
...
helm_install() {
  log "Installing AIQ AIRA blueprint (Option C: In-Cluster NIM)..."

  # Check if values file exists
  if [ ! -f "manifests/aiq-nim-values.yaml" ]; then
    log "ERROR: manifests/aiq-nim-values.yaml not found"
    log "This script deploys Option C with in-cluster NIM"
    exit 1
  fi
  ...
  if [ $? -eq 0 ]; then
    ...
    log "NOTE: NIM LLM pod will take 5-10 minutes to download and load the 49B model"
  else
...
helm_upgrade() {
  log "Upgrading AIQ AIRA deployment (Option C: In-Cluster NIM)..."
...
wait_for_deployment() {
  ...
  # Check for NIM LLM StatefulSet (Option C)
  if kubectl get statefulset aiq-nim-llm -n "$NAMESPACE" >/dev/null 2>&1; then
    log "  Waiting for NIM LLM (this may take 5-10 minutes to download and load the model)..."
    kubectl wait --for=condition=ready pod -l app.kubernetes.io/name=nim-llm -n "$NAMESPACE" --timeout=15m || {
      log "  NIM LLM: Still initializing (model loading in progress)"
      log "  You can check progress with: kubectl logs -n $NAMESPACE aiq-nim-llm-0 -f"
    }
  fi
...
do_setup_infra() {
  ...
  log ""
  log "Azure Monitor Workspace: $AZ_MONITOR_WORKSPACE_NAME"
  log "Grafana Instance: $GRAFANA_NAME"
  log "AKS Cluster: $CLUSTER_NAME"
  log ""
  log "Next steps:"
